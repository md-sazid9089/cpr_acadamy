...
test('SBA and MTF grading counts false as an answer and floors total scores at zero', () => {
  assert.deepEqual(gradePaper({ questions, negativeMarking: 25 }, { 'question-one': 'second', 'question-two': { first: false, second: false } }), { score: 1.75, totalMarks: 3, correctCount: 2, wrongCount: 1, skippedCount: 0, passMark: 70, passed: false });
  assert.equal(gradePaper({ questions, negativeMarking: 25 }, { 'question-one': 'first' }).score, 0);
});
...
test('exam attempts hide keys, preserve deadlines, freeze papers, and reject late answers', async () => {
  ...
  try {
    ...
    let response = await admin.request('POST', '/admin/exams', { courseId: course.id, title: 'Practice Paper', type: 'practice', questionType: 'mixed', durationMinutes: 10, scheduledAt: '2025-01-01T00:00:00Z', isPublished: true, questions: mixedPaperQuestions(), targetQuestionCount: 50 });
    ...
    assert.equal((await student.request('POST', `/exams/${exam.id}/answers`, { questionId: 'unknown', answer: 'second', version: 0 })).statusCode, 400);
    response = await student.request('POST', `/exams/${exam.id}/answers`, { questionId: 'question-one', answer: 'second', version: 0 });
